// The legacy Mesh label uses the first eight hex digits of SHA-256(session ID).
// Keep this synchronous so the browser and paginated server renderer agree.
const K = [
...
export function conversationTitle(session) {
  ...
  if (typeof title === 'string' && title.trim() && title !== 'Conversation sans titre') return title;
  const id = session.sourceSessionId || session.id;
  return id ? `Conversation ${shortId(String(id))}` : 'Conversation sans titre';
}
