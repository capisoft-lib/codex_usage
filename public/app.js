...
const I18N = {
  fr: {
    ...
    "search.placeholder": "Rechercher…", "model.all": "Tous les modèles", "conversation.untitled": "Conversation sans titre", "conversation.none": "Aucune conversation pour ces filtres.",
    ...
    "table.count": "{n} conversation{s}", "table.shown": "{shown} affichées sur {total}",
  ...
  en: {
    ...
    "search.placeholder": "Search…", "model.all": "All models", "conversation.untitled": "Untitled conversation", "conversation.none": "No conversations match these filters.",
    ...
    "table.count": "{n} conversation{s}", "table.shown": "Showing {shown} of {total}",
  ...
  de: {
    ...
    "search.placeholder": "Suchen…", "model.all": "Alle Modelle", "conversation.untitled": "Unbenannte Konversation", "conversation.none": "Keine Konversationen für diese Filter.",
    ...
    "table.count": "{n} Konversation{s}", "table.shown": "{shown} von {total} angezeigt",
...
function renderTable(sessions) {
  const filtered = sessions.filter((session) => !state.query || `${session.title} ${session.models.join(" ")}`.toLowerCase().includes(state.query.toLowerCase())).sort((a, b) => b.usage.totalTokens - a.usage.totalTokens);
  const visible = filtered.slice(0, 100);
  $("#conversationRows").innerHTML = visible.length ? visible.map((session) => {
    const cost = costOfCalls(session.calls); const model = [...new Set(session.calls.map((call) => call.model))].join(", ") || session.models.join(", ") || "unknown";
    return `<tr data-session-id="${escapeHtml(session.id)}"><td><div class="conversation-name">${escapeHtml(sessionTitle(session))}</div><div class="conversation-date">${formatDate(new Date(session.updatedAt))}</div></td><td><span class="model-pill">${escapeHtml(model)}</span></td><td>${session.exchanges}</td><td>${session.modelCalls}</td><td>${formatCompact(session.usage.totalTokens)}</td><td>${formatDuration(session.durationMs)}</td><td class="cost">${formatCost(cost.cost)}${cost.estimatedCalls ? " ≈" : ""}</td></tr>`;
  }).join("") : `<tr><td colspan="7" class="empty">${t("conversation.none")}</td></tr>`;
  $("#tableCount").textContent = filtered.length > visible.length ? t("table.shown", { shown: visible.length, total: filtered.length }) : t("table.count", { n: filtered.length, s: pluralSuffix(filtered.length) });
...
async function loadData(force = false) {
  ...
  try {
    ...
    state.data = await response.json(); populateModels(); render(); if (force) toast(t("refresh.done"));
...
$$('[data-period]').forEach((button) => button.addEventListener("click", () => { $$('[data-period]').forEach((item) => item.classList.remove("active")); button.classList.add("active"); state.period = button.dataset.period; render(); }));
$("#modelFilter").addEventListener("change", (event) => { state.model = event.target.value; render(); });
$("#searchInput").addEventListener("input", (event) => { state.query = event.target.value; renderTable(scopedSessions()); });
