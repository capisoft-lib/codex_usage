...
test("page API aggregates bounded batches, isolates owners, paginates and loads details on demand", async () => {
  ...
  try {
    ...
    const data=await response.json();assert.equal(data.pageData.totals.count,501);assert.equal(data.sessions.length,6);assert.equal(sessionQueries,2);
    ...
    assert.equal(list.sessions.length,20);assert.equal(list.pageData.total,501);assert.equal(list.pageData.page,2);
