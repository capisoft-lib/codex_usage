...
export async function POST(request: Request) {
  try {
    ...
    const statements = [database.prepare("UPDATE mesh_nodes SET last_sequence = ?, last_payload_hash = ?, last_seen = ?, last_generated_at = ?, privacy_json = ?, short_quota_json = ?, quota_json = ?, quota_history_json = ?, analyzer_version = ? WHERE id = ? AND last_sequence < ?")
      .bind(envelope.sequence, envelope.payloadHash, receivedAt, String(payload.generatedAt), JSON.stringify(payload.privacy), payload.shortQuota == null ? null : JSON.stringify(payload.shortQuota), payload.quota == null ? null : JSON.stringify(payload.quota), payload.quotaHistory == null ? null : JSON.stringify(payload.quotaHistory), Number(payload.analyzerVersion) || 0, envelope.nodeId, envelope.sequence)];
