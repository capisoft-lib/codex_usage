...
type NodeRow = { id: string; alias: string; enrolled_at: string; last_seen: string | null; revoked_at: string | null; privacy_json: string; quota_json: string | null; quota_history_json: string | null; analyzer_version: number };
type SessionRow = { node_id: string; snapshot_json: string };

export async function aggregateUsageForOwner(ownerId: string) {
  const database = db();
  const nodeResult = await database.prepare("SELECT id, alias, enrolled_at, last_seen, revoked_at, privacy_json, quota_json, quota_history_json, analyzer_version FROM mesh_nodes WHERE owner_id = ? ORDER BY alias").bind(ownerId).all<NodeRow>();
